...
class MealService {
  ...
  static Future<List<Meal>> getMealRecommendations({
    ...
    try {
      ...
      // If we got meals and have available ingredients, calculate relevance scores
      if (meals.isNotEmpty && availableIngredients != null && availableIngredients.isNotEmpty) {
        final scoredMeals = meals.map((meal) {
        ...
        await _saveMealsToStorage(scoredMeals);
        return scoredMeals;
      }
      
      // If we got meals without ingredient scoring
      if (meals.isNotEmpty) {
        // Save to local storage
      ...
      // If we reach here, we couldn't get meals from the API
      // Use fallback meals with ingredient matching if available
  ...
  static List<Meal> _parseMealsFromAiResponse(String content, String source) {
    try {      debugPrint('Parsing AI response for meal recommendations');
      ...
      // Remove markdown code block markers (```json, ```javascript, etc. and ```)
      cleanedContent = cleanedContent.replaceAll(RegExp(r'```(?:json|javascript)?\s*|\s*```', multiLine: true), '');
      ...
      // Strategy 1: Look for a well-formed JSON array
      ...
      // Strategy 2: Try parsing the entire content as a JSON array
      try {
      ...
      // Strategy 3: Look for individual JSON objects and combine them
      ...
      if (mealMatches.isNotEmpty) {
        ...
        for (final mealString in mealMatches) {
          try {
            ...
            // Try to fix this individual meal JSON
            try {
              final fixedMealString = _fixJsonString(mealString);
              final meal = jsonDecode(fixedMealString);
              meals.add(meal);
            } catch (fixError) {
              debugPrint('Could not fix individual meal JSON: $fixError');
      ...
      // Strategy 4: Apply aggressive JSON fixing and try again
      debugPrint('Attempting to fix malformed JSON');
      String fixedContent = _fixJsonString(cleanedContent);
      ...
      try {
        ...
        if (fixedJsonArrayMatch != null) {
          ...
          try {
            ...
            debugPrint('Error parsing fixed JSON array: $fixedJsonError');
          }
        }
      } catch (fixedJsonError) {
      ...
      // Strategy 5: As a last resort, try to extract key-value pairs directly from the text
      debugPrint('Attempting to extract meal data directly from text');
      final manuallyExtractedMeals = _extractMealsFromText(cleanedContent);
      
      if (manuallyExtractedMeals.isNotEmpty) {
        debugPrint('Successfully extracted ${manuallyExtractedMeals.length} meals manually from text');
        return _createMealsFromJson(manuallyExtractedMeals, source);
      }
  ...
  /// Fix common JSON formatting issues in a string
  static String _fixJsonString(String content) {
    String fixedContent = content;
    
    // Fix common JSON formatting issues
    // Find any unquoted property names (like nutrients: { calories: 400 })
    ...
    fixedContent = fixedContent.replaceAll(RegExp(r'[\u0000-\u001F]'), '');    // Add missing quotes around string values
    RegExp valuePattern = RegExp(r':\s*([\w\s\-\.]+)(,|\s*})');
    final matches = valuePattern.allMatches(fixedContent).toList();
    ...
    for (int i = matches.length - 1; i >= 0; i--) {
      ...
      if (double.tryParse(value ?? '') != null) {
        continue; // Leave numbers as is
      }
      
      // Replace the match with quoted value
      final start = match.start;
      final end = match.end;
      final replacement = ': "$value"$separator';
      fixedContent = fixedContent.substring(0, start) + replacement + fixedContent.substring(end);
    }
    
    return fixedContent;
  }

  /// Extract meal data directly from text when JSON parsing fails
  static List<Map<String, dynamic>> _extractMealsFromText(String content) {
    List<Map<String, dynamic>> meals = [];
    
    // Look for meal name patterns
    final nameMatches = RegExp(r'name["\s:]*([^"}\n,]+)', caseSensitive: false).allMatches(content);
    
    for (final nameMatch in nameMatches) {
      if (nameMatch.group(1) != null) {
        // Found a potential meal name
        final mealName = nameMatch.group(1)!.trim();
        final mealData = {
          'name': mealName,
          'description': '',
          'ingredients': <String>[],
        ...
        // Look for description near this name
        final descStartPos = content.indexOf(nameMatch.group(0)!) + nameMatch.group(0)!.length;
        final descSubstring = content.substring(descStartPos, _min(descStartPos + 500, content.length));
        final descMatch = RegExp(r'description["\s:]*([^"}\n,]+)', caseSensitive: false).firstMatch(descSubstring);
        
        if (descMatch != null && descMatch.group(1) != null) {
          mealData['description'] = descMatch.group(1)!.trim();
        }
        
        // Look for ingredients list
        final ingredientsMatch = RegExp(r'ingredients["\s:]*\[(.*?)\]', caseSensitive: false, dotAll: true).firstMatch(descSubstring);
        if (ingredientsMatch != null && ingredientsMatch.group(1) != null) {
        ...
        // Look for instructions list
        final instructionsMatch = RegExp(r'instructions["\s:]*\[(.*?)\]', caseSensitive: false, dotAll: true).firstMatch(descSubstring);
        if (instructionsMatch != null && instructionsMatch.group(1) != null) {
        ...
        // Look for nutrients
        final nutrientsMatch = RegExp(r'nutrients["\s:]*\{(.*?)\}', caseSensitive: false, dotAll: true).firstMatch(descSubstring);
        if (nutrientsMatch != null && nutrientsMatch.group(1) != null) {
          final nutrientsText = nutrientsMatch.group(1)!;
          final nutrientEntries = nutrientsText.split(',');
          
          Map<String, dynamic> nutrientsMap = {};
          
          for (final entry in nutrientEntries) {
            final keyValuePair = entry.split(':');
            if (keyValuePair.length == 2) {
              final key = keyValuePair[0].trim().replaceAll('"', '');
              final valueStr = keyValuePair[1].trim();
              final value = double.tryParse(valueStr) ?? 0.0;
              if (key.isNotEmpty) {
                nutrientsMap[key] = value;
              }
            }
          }
          
          mealData['nutrients'] = nutrientsMap;
        }
        
        // Only add meals that have at least a name and some ingredients
        if (mealName.isNotEmpty && (mealData['ingredients'] as List).isNotEmpty) {
          meals.add(mealData);
        }
      }
    }
  ...
  static List<Meal> _createMealsFromJson(List<dynamic> mealsJson, String source) {
    return mealsJson.map((mealJson) {
      ...
      return Meal(
        ...
        source: source,
        relevanceScore: 0.0, // Will be calculated later if needed
      );
    }).toList();
  }
  ...
  static List<Meal> _getFallbackMeals({String? mealType, List<String>? availableIngredients}) {
    ...
    // Use the IngredientService to calculate scores for each meal
    final scoredMeals = allPossibleMeals.map((meal) {
      final score = IngredientService.calculateIngredientMatchScore(
        meal.ingredients, 
        availableIngredients
      );
      
      // Return a copy of the meal with its relevance score
      return meal.copyWith(relevanceScore: score);
    }).toList();
    ...
    // Only return meals with at least some relevance (if available)
    // Otherwise return all meals
    final filteredMeals = scoredMeals.where((meal) => meal.relevanceScore > 0.1).toList();
    return filteredMeals.isNotEmpty ? filteredMeals : scoredMeals;
  }
  ...
  static Future<void> toggleFavorite(Meal meal) async {
    try {
      ...
      final mealIndex = meals.indexWhere((m) => m.id == meal.id);
      
      if (mealIndex >= 0) {
        final updatedMeal = meals[mealIndex].copyWith(isFavorite: !meals[mealIndex].isFavorite);
  ...
  static Future<void> recordMealFeedback(String mealId, bool liked, String? feedback) async {
    try {
      ...
      final feedbackData = await StorageService.loadData('meal_feedback') ?? {};
      ...
      // Save feedback
      await StorageService.saveData('meal_feedback', feedbackData);
      
      // Update meal relevance score if needed
      final meals = await loadMealRecommendations();
      ...
      if (mealIndex >= 0) {
        ...
        double newScore = meals[mealIndex].relevanceScore + scoreAdjustment;
        // Clamp score between 0 and 1
        newScore = newScore < 0 ? 0 : (newScore > 1 ? 1 : newScore);
        
        final updatedMeal = meals[mealIndex].copyWith(relevanceScore: newScore);
        meals[mealIndex] = updatedMeal;
        final mealsJson = meals.map((m) => m.toJson()).toList();
        await StorageService.saveData('meal_recommendations', mealsJson);
      }
